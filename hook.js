...
// Suppress Google Play Games / Google Sign-In popups
function installGoogleSignInBlocker() {
    if (typeof Java === "undefined" || !Java.available) return;
    Java.perform(function () {
        try {
            var GoogleServiceClient = Java.use("com.supercell.titan.GoogleServiceClient");
            GoogleServiceClient.isAvailable.implementation = function () {
                emit("[HOOK] GoogleServiceClient.isAvailable() -> false (Google login disabled)");
                return false;
            };
            GoogleServiceClient.forNative_signIn.implementation = function (val) {
                emit("[HOOK] GoogleServiceClient.forNative_signIn() -> suppressed");
                return;
            };
            GoogleServiceClient.signIn.implementation = function () {
                emit("[HOOK] GoogleServiceClient.signIn() -> suppressed");
                return;
            };
            ...
            emit("[HOOK] GoogleServiceClient blocker installed");
        } catch (e) {}

        try {
            var Activity = Java.use("android.app.Activity");
            Activity.startActivityForResult.overload("android.content.Intent", "int").implementation = function (intent, reqCode) {
                var cmp = intent.getComponent() ? intent.getComponent().flattenToString() : "";
                var act = intent.getAction() || "";
                var pkg = intent.getPackage() || "";
                var low = (cmp + " " + act + " " + pkg).toLowerCase();
                if (low.indexOf("google") !== -1 || low.indexOf("vending") !== -1 || low.indexOf("play.games") !== -1 || low.indexOf("signin") !== -1) {
                    emit("[HOOK] Blocked startActivityForResult: " + cmp + " (" + act + ")");
            ...
            Activity.startActivity.overload("android.content.Intent").implementation = function (intent) {
                ...
                var low = (cmp + " " + act + " " + pkg).toLowerCase();
                if (low.indexOf("google") !== -1 || low.indexOf("vending") !== -1 || low.indexOf("play.games") !== -1 || low.indexOf("signin") !== -1) {
                    emit("[HOOK] Blocked startActivity: " + cmp + " (" + act + ")");
                    return;
                }
                this.startActivity(intent);
            };
            emit("[HOOK] Activity Google launch blocker installed");
        } catch (e) {}

        try {
            var GApi = Java.use("com.google.android.gms.common.GoogleApiAvailability");
...
try { installGoogleSignInBlocker(); } catch (_) {}
