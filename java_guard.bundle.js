...
`;
  ...
  function installGoogleSignInBlocker() {
    if (typeof frida_java_bridge_default === "undefined" || !frida_java_bridge_default.available) {
      return;
    }
    frida_java_bridge_default.performNow(() => {
      try {
        const GoogleServiceClient = frida_java_bridge_default.use("com.supercell.titan.GoogleServiceClient");
        GoogleServiceClient.isAvailable.implementation = function () {
          send("[JAVA-GUARD] GoogleServiceClient.isAvailable() -> false (Google login disabled)");
          return false;
        };
        GoogleServiceClient.forNative_signIn.implementation = function (val) {
          send("[JAVA-GUARD] GoogleServiceClient.forNative_signIn() -> suppressed");
          return;
        };
        GoogleServiceClient.signIn.implementation = function () {
          send("[JAVA-GUARD] GoogleServiceClient.signIn() -> suppressed");
          return;
        };
        GoogleServiceClient.forNative_isAuthenticated.implementation = function () { return; };
        GoogleServiceClient.isAuthenticated.implementation = function () { return; };
        GoogleServiceClient.forNative_signOut.implementation = function () { return; };
        GoogleServiceClient.signOut.implementation = function () { return; };
        GoogleServiceClient.isSignedIn.implementation = function () { return false; };
        GoogleServiceClient.getAuthCode.implementation = function () { return ""; };
        GoogleServiceClient.getPlayerId.implementation = function () { return ""; };
        GoogleServiceClient.getPlayerDisplayName.implementation = function () { return ""; };
        send("[JAVA-GUARD] GoogleServiceClient blocker installed");
      } catch (e) {
        send("[JAVA-GUARD] GoogleServiceClient hook note: " + e);
      }
      ...
      try {
        ...
        Activity.startActivityForResult.overload("android.content.Intent", "int").implementation = function (intent, reqCode) {
          const cmp = intent.getComponent() ? intent.getComponent().flattenToString() : "";
          const act = intent.getAction() || "";
          const pkg = intent.getPackage() || "";
          const low = (cmp + " " + act + " " + pkg).toLowerCase();
          if (low.includes("google") || low.includes("vending") || low.includes("play.games") || low.includes("signin")) {
            send("[JAVA-GUARD] Blocked startActivityForResult: " + cmp + " (" + act + ")");
            this.onActivityResult(reqCode, 0, null);
            return;
          }
        ...
        Activity.startActivity.overload("android.content.Intent").implementation = function (intent) {
          ...
          const low = (cmp + " " + act + " " + pkg).toLowerCase();
          if (low.includes("google") || low.includes("vending") || low.includes("play.games") || low.includes("signin")) {
            send("[JAVA-GUARD] Blocked startActivity: " + cmp + " (" + act + ")");
            return;
          }
          this.startActivity(intent);
        };
        send("[JAVA-GUARD] Activity Google launch blocker installed");
      } catch (e) {
        send("[JAVA-GUARD] Activity hook note: " + e);
      }

      try {
        const GApi = frida_java_bridge_default.use("com.google.android.gms.common.GoogleApiAvailability");
        GApi.isGooglePlayServicesAvailable.overload("android.content.Context").implementation = function (ctx) {
          return 1;
        };
        send("[JAVA-GUARD] GoogleApiAvailability hook installed");
      } catch (_) {}
    });
  }
  var googleClientHooked = false;
  function hookGoogleServiceClientInLoaders() {
    if (googleClientHooked || typeof frida_java_bridge_default === "undefined" || !frida_java_bridge_default.available) return true;
    try {
      const loaders = frida_java_bridge_default.enumerateClassLoadersSync();
      for (const loader of loaders) {
        try {
          const factory = frida_java_bridge_default.ClassFactory.get(loader);
          const GoogleServiceClient = factory.use("com.supercell.titan.GoogleServiceClient");
          GoogleServiceClient.isAvailable.implementation = function () {
            send("[JAVA-GUARD] GoogleServiceClient.isAvailable() -> false (Google login disabled)");
            return false;
          };
          ...
          GoogleServiceClient.getPlayerId.implementation = function () { return ""; };
          GoogleServiceClient.getPlayerDisplayName.implementation = function () { return ""; };
          googleClientHooked = true;
          send("[JAVA-GUARD] GoogleServiceClient hooked via ClassLoader!");
          return true;
        } catch (_) {}
      }
    } catch (_) {}
  ...
  discoverReportGuard();
  installGoogleSignInBlocker();
  hookGoogleServiceClientInLoaders();
  ...
  var directGuardTimer = setInterval(() => {
    ...
    if ((reportGuardInstalled && googleClientHooked) || Date.now() - directGuardStartedAt > 1e4) {
